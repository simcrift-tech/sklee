...
DEFAULT_IOT_DEVICES = [
    {
        'name': 'Lobby_TV', 
        'id': 'ebe3679542e94a90bfv2n6',  # [수정필요] 실제 Device ID
        'key': '+^A$*TKBjHPD:e.v',   # [수정필요] 실제 Local Key
        'ip': '192.168.0.4',        # [수정필요] 실제 IP 주소
        'version': 3.4,              # 프로토콜 버전
        'icon': 'tv',
        'group': '1층 로비',
        'status': False
...
def init_iot_devices():
    ...
    for config in device_list:
        ...
        # [수정] 테스트를 위해 더미 데이터 체크 로직을 주석 처리함 (무조건 시도)
        # if not dev_id or 'dummy' in dev_id or not dev_ip: 
        #     print(f"   ⚠️  스킵됨: {config.get('name')} (유효하지 않은 정보)")
        #     continue
        
        print(f"   ⏳ 객체 생성 시도: {config['name']} ({dev_ip})...")
        try:
            # 타임아웃을 0.5초로 줄여서 부팅 속도 향상 (장비가 없으면 어차피 실패하므로)
            ...
            print(f"   ✅ 가상 객체 등록 완료: {config['name']}")
...
def sync_library():
    ...
    if added_count > 0:
        ...
        print(f"📂 [System] 총 {added_count}개의 새 파일 등록 완료.")
    else:
...
def get_data():
    ...
    if 'storage' not in data:
        	data['storage'] = {}   # 'storage' 키를 빈 딕셔너리로 생성
...
@app.route('/suplay')
def suplay_page():
	return render_template('suplay.html')
@app.route('/m')
