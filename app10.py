...
def init_iot_devices():
    ...
    for config in device_list:
        ...
        # [테스트 모드] 더미 데이터 체크 주석 처리 (장비 없이도 등록 시도)
...
if __name__ == '__main__':
    ...
    print(f"👉 접속 주소: {url}")
    print(f"📺 화면 송출: {url}/stream_screen")
    print("="*50 + "\n")

    # 브라우저 자동 열기: 짧은 지연 후 타이머로 실행 (리로더 중복 방지 위해 use_reloader=False 권장)
    def _open_browser():
        try:
            webbrowser.open(url)
        except Exception as e:
    ...
    # 디버그 모드에서 reloader가 브라우저를 두 번 열 수 있으므로 use_reloader=False 권장
