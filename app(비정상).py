    ...
    if not os.path.exists(DATA_FILE):
        ...
        try:
            with open(DATA_FILE, 'r', encoding='utf-8') as f:
                ...
                if 'library' not in data: data['library'] = [] # 라이브러리 키가 없으면 생성
                return data
        except: return {}
...
def init_iot_devices():
    ...
    for config in device_list:
        ...
        try:
            d = tinytuya.OutletDevice(dev_id=dev_id, address=dev_ip, local_key=dev_key, connection_timeout=2)
            d.set_version(float(config.get('version', 3.3)))
            d.set_socketPersistent(True) 
            iot_devices_map[dev_id] = { 'device': d, 'name': config['name'], 'ip': dev_ip }
            print(f"   ✅ 연결 객체 생성: {config['name']}")
        except Exception as e:
            print(f"      ❌ 객체 생성 실패: {e}")
    print("🔌 [System] IoT 초기화 완료.")
    print("="*50 + "\n")

# --- [기능 3] 폴더 스캔 및 라이브러리 동기화 (추가됨) ---
def sync_library():
    print("📂 [System] Uploads 폴더 스캔 중...")
    data = load_data_from_file()
    library = data.get('library', [])
    
    # 현재 라이브러리에 있는 파일명 목록 (중복 방지용)
    existing_filenames = set(item['realFilename'] for item in library)
    
    if not os.path.exists(UPLOAD_FOLDER):
        os.makedirs(UPLOAD_FOLDER)
        
    disk_files = os.listdir(UPLOAD_FOLDER)
    added_count = 0
    
    for filename in disk_files:
        # 지원하는 파일이고, 라이브러리에 없다면 추가
        if allowed_file(filename) and filename not in existing_filenames:
            ...
            # 라이브러리 항목 생성
            new_item = {
                'id': int(time.time() * 1000) + added_count, # 고유 ID 생성
                'title': filename,
            ...
            existing_filenames.add(filename) # 중복 체크용 갱신
            added_count += 1
            print(f"   ➕ 파일 감지 및 등록: {filename}")
    ...
    if added_count > 0:
        ...
        print(f"📂 [System] 총 {added_count}개의 새 파일이 라이브러리에 등록되었습니다.")
    else:
        print("📂 [System] 새로운 파일이 없습니다.")
        ...
        if dev_id in iot_devices_map:
            try:
                ...
                if status_data and 'dps' in status_data:
                    dps = status_data['dps']
                    if '1' in dps: is_on = bool(dps['1'])
                    elif '20' in dps: is_on = bool(dps['20'])
                    else: is_on = True
            except: is_on = False
        status_list.append({
            'id': dev_id, 'name': conf.get('name'), 'icon': conf.get('icon', 'tv'),
            'isOn': is_on, 'schedule': conf.get('schedule', {'enabled': False})
        })
...
def control_iot_device():
    ...
    print(f"📡 [Control] 요청: ID={dev_id}, Action={action}")
    success = False
    error_msg = ""
    if dev_id in iot_devices_map:
        try:
            device = iot_devices_map[dev_id]['device']
            if action == 'on': res = device.turn_on()
            else: res = device.turn_off()
            if res and 'Error' not in str(res): 
                success = True
                time.sleep(0.5)
            else: error_msg = f"장비 응답 에러: {res}"
        except Exception as e: error_msg = f"통신 예외: {str(e)}"
    else: error_msg = "초기화된 장비 객체 없음"
    
    if success:
        data = load_data_from_file()
        for d in data.get('iot_devices', []):
            if d['id'] == dev_id:
                d['status'] = (action == 'on')
                break
...
def save_iot_schedule():
    ...
    for d in data.get('iot_devices', []):
        if d['id'] == dev_id:
            d['schedule'] = {'on_time': req.get('on_time'), 'off_time': req.get('off_time'), 'enabled': req.get('enabled')}
            break
    with open(DATA_FILE, 'w', encoding='utf-8') as f: json.dump(data, f, ensure_ascii=False, indent=4)
    return jsonify({'success': True})
...
if __name__ == '__main__':
    ...
    # 2. IoT 장비 연결 초기화
    try: init_iot_devices()
    except Exception as e: print(f"IoT 초기화 오류: {e}")
